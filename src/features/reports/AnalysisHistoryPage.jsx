            ...
            {['ALL', 'Passed', 'Warning', 'Failed'].map(st => (
              <button
                ...
                className={`px-4 py-1.5 rounded-lg font-medium transition-all tracking-wide ${
                  statusFilter === st ? 'bg-theme-accent-yellow text-theme-text-primary shadow-md' : 'text-theme-text-secondary hover:text-theme-text-primary'
                }`}
              >
                {st}
              </button>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-white/5 text-xs">
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={7} className="py-16 text-center text-theme-text-secondary/70 text-sm">
                ...
                filtered.map((item) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-6 text-theme-text-secondary whitespace-nowrap">
                      {new Date(item.date || Date.now()).toLocaleDateString()}{' '}
                      <span className="text-theme-text-secondary/70 text-[11px]">
                        {new Date(item.date || Date.now()).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                      </span>
                    </td>
                    <td className="py-4 px-6 font-medium text-theme-text-primary">
                      {item.agentName || 'Support Agent #42'}
                    </td>
                    <td className="py-4 px-6 text-theme-text-secondary font-mono truncate max-w-xs">
                      {item.aiModelUsed} <span className="text-theme-text-secondary/70">({item.promptVersion})</span>
                    </td>
                    <td className="py-4 px-6">
                      ...
                      <span className="text-[10px] text-theme-text-secondary/70">/100</span>
                    </td>
                    <td className="py-4 px-6">
                      {item.status === 'Passed' && (
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold bg-emerald-500/15 text-emerald-400">
                          <CheckCircle2 className="w-3 h-3" /> Passed
                        </span>
                      )}
                      {item.status === 'Warning' && (
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold bg-amber-500/15 text-amber-400">
                          <AlertTriangle className="w-3 h-3" /> Warning
                        </span>
                      )}
                      {item.status === 'Failed' && (
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold bg-red-500/15 text-red-400">
