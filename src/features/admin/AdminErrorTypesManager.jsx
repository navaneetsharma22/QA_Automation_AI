  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-200">
      <div className="bg-theme-main border border-theme-border rounded-2xl p-6">
        ...
        <form onSubmit={handleAddErrorType} className="grid grid-cols-1 md:grid-cols-12 gap-6 bg-theme-input p-6 rounded-xl border border-theme-border/50">
          <div className="md:col-span-4 space-y-2">
            <label className="text-xs font-bold text-theme-secondary uppercase tracking-wider">Error Type Name</label>
            <select 
              value={name}
              onChange={(e) => setName(e.target.value)}
              className="w-full bg-theme-main border border-theme-border text-theme-primary text-sm rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500 transition-colors appearance-none cursor-pointer" 
            >
          ...
          <div className="md:col-span-8 space-y-2">
            ...
            <textarea 
              ...
              className="w-full bg-theme-main border border-theme-border text-theme-primary text-sm rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500 transition-colors resize-none" 
            />
          </div>
          ...
          <div className="md:col-span-12 flex justify-end">
            <button 
              ...
              className="flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-theme-primary font-medium rounded-xl transition-all shadow-sm"
            >
              {isSubmitting ? 'Saving...' : (
                <>
        ...
        <div className="overflow-x-auto">
          {loading ? (
            ...
            <table className="w-full text-sm text-left">
              <thead className="text-xs text-theme-secondary uppercase bg-theme-input border-b border-theme-border">
                <tr>
              ...
              <tbody className="divide-y divide-[#1F2937]">
                {errorTypes.map((et, idx) => (
                  <tr key={et.id || idx} className="hover:bg-theme-input/50 transition-colors">
          ...
          <div className="bg-theme-main border border-theme-border rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-theme-border flex items-center justify-between">
              ...
              <button 
                ...
                className="text-theme-secondary hover:text-theme-primary p-1 rounded-lg hover:bg-theme-card transition-colors"
              >
            ...
            <form onSubmit={handleUpdateErrorType} className="p-6 space-y-6">
              <div className="space-y-2">
                ...
                <select 
                  ...
                  className="w-full bg-theme-input border border-theme-border text-theme-primary text-sm rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500 transition-colors appearance-none cursor-pointer" 
                >
                  <option value="" disabled>Select Error Type</option>
                  <option value="AHT (Average Handle Time)">AHT (Average Handle Time)</option>
                  <option value="ART (Agent Response Time)">ART (Agent Response Time)</option>
              ...
              <div className="space-y-2">
                ...
                <textarea 
                  ...
                  className="w-full bg-theme-input border border-theme-border text-theme-primary text-sm rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500 transition-colors resize-none" 
                />
              </div>
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-theme-border">
                <button 
                  type="button"
                  onClick={() => setIsEditModalOpen(false)}
                  className="px-6 py-2.5 bg-transparent hover:bg-theme-card text-theme-primary font-medium rounded-xl transition-all border border-theme-border"
                >
                  Cancel
                </button>
                <button 
                  type="submit"
                  disabled={isUpdating}
                  className="flex items-center gap-2 px-6 py-2.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-theme-primary font-medium rounded-xl transition-all shadow-sm"
                >
                  {isUpdating ? 'Updating...' : (
                    <>
