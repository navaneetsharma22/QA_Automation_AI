  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-start justify-center pt-20 px-4">
      <div className="w-full max-w-xl bg-[#111827] border border-[#1F2937] rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="max-h-80 overflow-y-auto py-2 px-2 divide-y divide-[#1F2937]/50">
          {filtered.length === 0 ? (
            <div className="py-8 text-center text-sm text-gray-500">No matching commands found.</div>
          ) : (
            filtered.map(item => {
              ...
              return (
                <button
                  ...
                  className="w-full flex items-center justify-between px-3 py-3 rounded-xl hover:bg-[#1F2937] text-left transition-colors text-sm text-gray-200 group"
                >
                  <div className="flex items-center gap-3">
                    <Icon className="w-4 h-4 text-blue-400" />
                    <span>{item.label}</span>
                  </div>
                  <span className="text-[11px] px-2 py-0.5 rounded bg-[#0B1020] text-gray-400 border border-[#1F2937]">
                    {item.category}
                  </span>
                </button>
              );
        ...
        <div className="px-4 py-2.5 bg-[#0B1020] border-t border-[#1F2937] flex items-center justify-between text-[11px] text-gray-400 font-mono">
